...
def PCellAnyArc(radius=10.0, width=0.5, angle_deg=90.0, layer=1, name="QUARTER_ARC"):
    """
    ...
    npts = max(16, int(0.5 * abs(angle_deg)))  # sample density
    ...
    p0 = Port("W", x=pts[0][0], y=pts[0][1], angle=0.0, width=width, layer=layer)
    p1 = Port("E", x=pts[-1][0], y=pts[-1][1], angle=angle_deg, width=width, layer=layer)
